...
public abstract class Loader<Return> {
  ...
  @WorkerThread @Nullable protected abstract Return buildObject(@NonNull Cursor cursor);
  ...
  class LoadTask extends AsyncTask<Object, Return, List<Return>> {
    ...
    @Override protected List<Return> doInBackground(Object... params) {
      isExecuting = true;
    ...
    @SafeVarargs @WorkerThread public final void oneLoaded(Return... progress) {
  ...
  @UiThread protected void sort(List<Return> data) {
    //Do nothing
  }

  private List<Return> currentData = new ArrayList<>();
  private TaskListener<Return> mVerifyListener = new TaskListener<Return>() {
    @Override public void onOneLoaded(Return item, int pos) {
            /*if (!currentData.contains(item)) */
      for (TaskListener<Return> listener : mListeners) listener.onOneLoaded(item, pos);
    }

    @Override public void onCompleted(List<Return> result) {
  ...
  /**
   * Make a content observer to be registered/unregistered. Do <b>NOT</b> pass variables. Whatever
   * is returned here will be set in a final variable and accessed from there
   *
   * @return The content observer
   */
  @Nullable @UiThread protected ContentObserver getObserver() {
    return null;
  }

  @UiThread public void update(final List<Return> currentData) {
  ...
  /**
   * Registers a {@link ContentObserver} and removes any previous calls to {@link
   * #unregisterMediaStoreListener()}
   */
  @UiThread public void registerMediaStoreListener() {
    if (mObserver != null) {
   ...
   */
  @UiThread protected void unregisterMediaStoreListenerTemporarily() {
    if (mObserver != null) {
  ...
  /**
   * Same as {@link #unregisterMediaStoreListenerTemporarily()} except it also makes sure the async
   * task doesn't ever register the listener again until {@link #registerMediaStoreListener()} is
   * called from an outside source
   */
  @UiThread public void unregisterMediaStoreListener() {
    if (mObserver != null) {
      unregisterMediaStoreListenerTemporarily();
      mObserverLock = true;
    }
   ...
   *
   * @param <Return> What type of variable should be passed to the listener. When extending {@link
   * Loader}, you will specify what this should be
   */
  ...
  @UiThread public void addListener(TaskListener<Return> listener) {
   ...
   */
  @WorkerThread public void notifyOneLoaded(Return progress) {
